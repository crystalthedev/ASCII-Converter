...
![Algorithm](https://img.shields.io/badge/Algorithm-red?style=for-the-badge)
   - **Изображение делится на блоки (pixel size)**
   - **Для каждого блока берется центральный пиксель**
   - **Вычисляется яркость пикселя (luminance)**
   - **Применяются контраст и яркость**
   - **Выбирается **случайный символ** из набора**
   - **Символ окрашивается в зависимости от режима:**
